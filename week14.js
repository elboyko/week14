...
const myFirstArray = ["cat", "dog", true, 33, null, 0];
...
const mySecondArray = ['Moskow', 'Novosibirsk', 'Berdsk'];
...
const myInformation = ['Helen', 18, 11, 'Scorpio'];
...
let newCountries = ['Austria', 'Belgium', 'Netherlands', 'Czechia', 'Croatia'];
console.log(newCountries[newCountries.length - 1]);
...
russianCities = [...newCities, ...russianCities];
...
function sum() {
   let sumTwoItems = numbers[0] + numbers[numbers.length - 1];
   console.log(sumTwoItems);
}
sum();
...
const filterEvenNumbers = numbers.filter((item) => {
   return item % 2 == 0
})
